...
import { updateFolder } from '@/firebaseFunctions/user/folder'
...
const UpdateFolderModal = ({ folder, onCloseClick }: UpdateFolderModalProps) => {
    ...
    const handleUpdateFolder = async () => {
        if (folder) {
            await updateFolder(userId, folder.folder_id, {
                title: folderName,
                icon: folderIcon
            })
            onCloseClick()
            ...
            <div className='w-[900px] h-[450px] bg-white py-[25px] rounded-[25px] overflow-hidden flex flex-col gap-[20px]'>
                    ...
                    <div className='flex flex-1 flex-col gap-[15px] justify-between'>
                        ...
                        <div className='flex flex-wrap gap-[15px] justify-between items-center'>
                            {folderIcons.map((icon, index) => (
                                <button
                                    onClick={() => setFolderIcon(index)}
                                    key={index}
                                    className={`w-[50px] h-[50px] rounded-full border-borderColor border-[2px] border-solid flex justify-center items-center ${folderIcon === index ? "bg-primaryColorLight" : "bg-accentColorLight"}`}
                                >
                ...
                <div className='flex justify-end items-center gap-[15px] px-[20px] pb-[20px]'>
                    <button
                        ...
                        className='flex-1 border-borderColor border-[3px] border-solid px-[20px] py-[10px] rounded-[15px] font-semibold text-[16px] text-primaryColorLight hover:border-primaryColorLight transition-all duration-300'
                    >
                        Cancel
                    </button>
                    <button
                        onClick={handleUpdateFolder}
                        className='flex-1 bg-primaryColorLight text-white px-[20px] py-[10px] rounded-[15px] font-semibold text-[16px]'
                    >
                        Update
                    </button>
                </div>
