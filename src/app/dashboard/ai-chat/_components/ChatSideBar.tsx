...
import NewAiChatModal from './modals/NewAiChatModal'
        ...
        return (
            <button className={`flex flex-col gap-[2px] border-[3px] border-solid rounded-[15px] ${selectedChat?.chat_id === chat.chat_id ? 'border-primaryColorLight' : 'border-transparent'}  `} onClick={() => setSelectedChat(chat)}>
                <div className='flex justify-between items-center gap-[50px] overflow-hidden  px-[18px] py-[15px] rounded-t-[15px] bg-accentColorLight w-full text-left'>
                    <p className={`font-medium text-[16px] max-h-[48px] overflow-hidden w-full  ${chat.conversation[chat.conversation.length - 1].message_type == "response" && chat.translation_config ? "font-urdu line-clamp-1 leading-[50px] text-right " : "text-ellipsis line-clamp-2"} `}>{chat.conversation[chat.conversation.length - 1].text}</p>
    ...
    return (
        <div className='basis-[30%] bg-white rounded-2xl '>
            <div className='shadow-normal bg-white h-full rounded-[15px] py-[20px] flex flex-col gap-[15px]'>
                    ...
                    <div className='flex justify-start items-center gap-[15px] relative '>
                        ...
                        }} />
                        {
                            newChatModalOpen && (
                                ...
                                <NewAiChatModal onClose={() => setNewChatModalOpen(false)} />
                ...
                <div className='flex flex-col gap-[15px] px-[20px] max-h-[69vh] overflow-y-scroll'>
